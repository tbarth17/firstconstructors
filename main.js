...
function Dog(x) {
  'use strict';
   ...
   this.color = 'none'
    ...
    this.status = 'normal'
  if(x.hungry === undefined) {
    this.hungry = true
  } else {this.hungry = x.hungry};

  };

function Human(y) {
  y = y || {};
  if (y.cool === undefined) {this.cool = false
 } else {this.cool = y.cool};
}

Human.prototype.pet = function(i){
  i.status = 'happy';
};

Human.prototype.feed = function(i) {
  i.hungry = false;
};
